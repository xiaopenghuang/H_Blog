...
export default function HeroProfileCard() {
  return (
    <div className="card p-5 sm:p-6 md:p-8 shadow-card">
      <div>
        {/* Avatar */}
        <div className="flex justify-center mb-4 sm:mb-5">
          <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-full border-2 border-border overflow-hidden relative">
            {siteConfig.author.avatar.startsWith('http') || siteConfig.author.avatar.startsWith('/') ? (
              ...
              <div className="w-full h-full flex items-center justify-center bg-surface">
                <span className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground">{siteConfig.author.avatar}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="text-center mb-4 sm:mb-5">
          ...
          <p className="text-sm sm:text-base text-foreground-secondary font-medium">{siteConfig.author.role}</p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2 mb-5 sm:mb-6">
          {siteConfig.techStack.map((tech) => (
            <span key={tech} className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-secondary text-foreground-secondary">
              {tech}
            </span>
          ))}
        </div>
        ...
        <div className="flex justify-center gap-2 sm:gap-3 mb-4 sm:mb-5">
          <a
            ...
            className="w-10 h-10 rounded-lg bg-secondary hover:bg-accent/10 flex items-center justify-center text-foreground-secondary hover:text-accent transition-smooth"
            aria-label="GitHub"
          >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="block w-full py-3 rounded-xl text-center text-base font-bold bg-accent text-white hover:bg-accent-light transition-smooth"
        >
          Follow on GitHub       </a>
      </div>
    </div>
